...
#spacing表示在x、y、z方向上相邻像素之间的实际物理距离

# 用于确定何时认为数据在z方向上具有明显的异性
...
def resample_data_or_seg_to_spacing(data: np.ndarray,
    ...
    if force_separate_z is not None:  # true/false
...
def resample_data_or_seg(data: np.ndarray, new_shape: Union[Tuple[float, ...], List[float], np.ndarray],
    ...
    :param is_seg: data 是否为分割图像
    :param axis:    用于指定在哪个轴上进行重采样
    :param order:   重采样的阶数
    :param do_separate_z: 是否在z轴上分别重采样
    ...
    shape = np.array(data[0].shape) # (x,y,z)
    new_shape = np.array(new_shape)
    if np.any(shape != new_shape):
        data = data.astype(float)
        if do_separate_z:  # 对每个通道分别进行重采样，并在z轴上应用不同的重采样阶数。
            # print("separate z, order in z is", order_z, "order inplane is", order)
            assert len(axis) == 1, "only one anisotropic axis supported"
            axis = axis[0]
            if axis == 0:
                new_shape_2d = new_shape[1:] # shape = (y,z)
            elif axis == 1:
                new_shape_2d = new_shape[[0, 2]] # shape = (x,z)
            else:
                new_shape_2d = new_shape[:-1] #shape = (x,y)

            reshaped_final_data = []
            for c in range(data.shape[0]):  # 遍历通道维度
                reshaped_data = []
                ...
                if shape[axis] != new_shape[axis]:  # 原始数据的形状在指定轴上与新形状不匹配
                    ...
                    if not is_seg or order_z == 0: # 非分割图像或order_z ==0,对整个reshaped_data进行插值操作
                        reshaped_final_data.append(map_coordinates(reshaped_data, coord_map, order=order_z,
                                                                   mode='nearest')[None])
                    else: # 它按照不同的标签值将reshaped_data进行分割，并对每个分割部分进行插值，最后将它们合并成最终的重采样结果。
            ...
            reshaped = []
            for c in range(data.shape[0]): # 遍历通道维度
                reshaped.append(resize_fn(data[c], new_shape, order, **kwargs)[None]) #加[none]以便后续的堆叠操作
